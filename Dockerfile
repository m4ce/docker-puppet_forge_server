...
RUN cd /tmp && curl -L -O http://github.com/unibet/puppet-forge-server/archive/master.tar.gz && tar xzf master.tar.gz && cd puppet-forge-server-master && gem build puppet-forge-server.gemspec && bundle install
