...
RUN git clone http://github.com/unibet/puppet-forge-server /tmp/puppet-forge-server
RUN cd /tmp/puppet-forge-server && bundle install
RUN cd /tmp/puppet-forge-server && gem build puppet-forge-server.gemspec && gem install --local *.gem
